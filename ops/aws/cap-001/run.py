#!/usr/bin/env python3
"""Plan or execute the immutable EXP-2026-003 Graviton session."""
...
PRIOR_SPEND_USD = 1.4527
EXPERIMENT_TOKEN = "exp-2026-003-capacity"
...
def main() -> int:
    ...
    payload = {
        ...
        "runtime_checkpoint": str(
            root.parent / "result-first-bakeoff/evidence/checkpoints/runtime-checkpoints.tar.gz"
        ),
    }
    if not args.execute:
    ...
    checkpoint = root.parent / "result-first-bakeoff/evidence/checkpoints/runtime-checkpoints.tar.gz"
    if not checkpoint.is_file():
        raise SystemExit(f"verified runtime checkpoint missing: {checkpoint}")
    result = execute_aws_run(
        ...
        project=root,
        output_dir=root / "ops/evidence/EXP-2026-003",
        guest_template=root / "ops/aws/cap-001/guest.sh",
        immutable_assets={"RUNTIME_CHECKPOINTS": checkpoint},
        poll_seconds=15,
    )
