...
[[ "${EXPERIMENT_APPROVAL_TOKEN:-}" == "exp-2026-017-memory-confirmation" ]] || exit 64
...
cp ops/experiments/EXP-2026-017.json "$RESULTS/experiment.json"
cp ops/aws/tuning-003/protocol.json "$RESULTS/protocol.json"

apt-get update -qq
DEBIAN_FRONTEND=noninteractive apt-get install -y -qq python3-dev python3-venv curl libmimalloc-dev
...
"$ROOT/venv/bin/python" scripts/run_runtime_tuning_015.py \
  ...
  --protocol ops/aws/tuning-003/protocol.json --mimalloc-library "$MIMALLOC" \
