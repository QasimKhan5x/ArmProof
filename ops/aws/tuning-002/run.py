#!/usr/bin/env python3
"""Plan or execute the preregistered THP isolation experiment."""
...
PRIOR_SPEND_USD = 14.3208
EXPERIMENT_TOKEN = "exp-2026-016-thp-isolation"
...
def main() -> int:
    ...
    plan = make_plan(
        "EXP-2026-016", instance_type="c8g.4xlarge", region="us-east-1",
        maximum_runtime_minutes=35, volume_gib=60,
        prior_spend_usd=PRIOR_SPEND_USD, now=started,
    )
    if not args.execute:
        print(json.dumps({"plan": plan.payload(), "approval_token": plan.approval_token()}, indent=2, sort_keys=True))
        return 0
    ...
    result = execute_aws_run(
        ...
        plan=plan, approval_token=args.approval_token,
        experiment_token=EXPERIMENT_TOKEN, project=root,
        output_dir=root / "ops/evidence/EXP-2026-016",
        guest_template=root / "ops/aws/tuning-002/guest.sh",
        immutable_assets={"RUNTIME_CHECKPOINTS": checkpoints}, poll_seconds=15,
    )
