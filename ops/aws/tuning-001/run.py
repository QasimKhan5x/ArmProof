#!/usr/bin/env python3
"""Plan or execute the preregistered runtime-tuning experiment."""
...
PRIOR_SPEND_USD = 13.4872
EXPERIMENT_TOKEN = "exp-2026-015-runtime-tuning"
...
def main() -> int:
    ...
    if not checkpoints.is_file():
        raise SystemExit(f"immutable prerequisite missing: {checkpoints}")
    result = execute_aws_run(
        ...
        project=root,
        output_dir=root / "ops/evidence/EXP-2026-015",
        guest_template=root / "ops/aws/tuning-001/guest.sh",
        immutable_assets={"RUNTIME_CHECKPOINTS": checkpoints},
        poll_seconds=15,
    )
