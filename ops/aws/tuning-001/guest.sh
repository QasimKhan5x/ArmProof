...
[[ "${EXPERIMENT_APPROVAL_TOKEN:-}" == "exp-2026-015-runtime-tuning" ]] || exit 64
...
cp ops/experiments/EXP-2026-015.json "$RESULTS/experiment.json"
cp ops/aws/tuning-001/protocol.json "$RESULTS/protocol.json"

apt-get update -qq
DEBIAN_FRONTEND=noninteractive apt-get install -y -qq \
  python3-dev python3-venv curl linux-tools-common linux-tools-generic
...
printf '%s\n' "$MIMALLOC" > "$RESULTS/mimalloc-library.txt"

export PYTHONPATH="$WORK/src"
MIMALLOC_ARGS=()
if [[ -n "$MIMALLOC" ]]; then
  MIMALLOC_ARGS=(--mimalloc-library "$MIMALLOC")
fi
set +e
"$ROOT/venv/bin/python" scripts/run_runtime_tuning_015.py \
  --model-source "$MODEL_SOURCE" \
  --output "$RESULTS/tuning" \
  --protocol ops/aws/tuning-001/protocol.json \
  "${MIMALLOC_ARGS[@]}" | tee "$RESULTS/tuning.stdout.json"
TUNING_STATUS=${PIPESTATUS[0]}
set -e
