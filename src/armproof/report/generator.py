...
if(comparison&&Number.isFinite(comparison.metrics.enabled_kai_cycle_callchain_share))document.getElementById('arm-cycle-share').textContent=(comparison.metrics.enabled_kai_cycle_callchain_share*100).toFixed(1)+'%';
...
if(verification){const checked=verification.checksums.checked+(verification.reproduction_checksums?verification.reproduction_checksums.checked:0);const scope=verification.reproduction_checksums?'primary and fresh-instance confirmation bundles':'the evidence bundle';document.getElementById('verification-detail').textContent=`${checked} files verified across ${scope}. The comparison and decision were derived from bound evidence, not accepted as input.`;}
