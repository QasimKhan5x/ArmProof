...
def generate_report(
    ...
    *,
    ...
    if verification is not None:
        ...
        runtime_memory = verification.get("runtime_memory")
        if (
            ...
            or (
                ...
                and (
                    runtime_memory != summary.get("runtime_memory")
                    or runtime_memory.get("passed") is not True
                    or runtime_memory.get("confirmation_passes")
                    != runtime_memory.get("confirmation_windows")
                    or runtime_memory.get("simplification_failures")
                    != runtime_memory.get("simplification_windows")
                    or runtime_memory.get("outputs_equivalent") is not True
                    or runtime_memory.get("internal_checksummed_files", 0) < 1
                )
            )
            or (
                summary.get("runtime_memory") is not None
...
.metric-band{grid-template-columns:repeat(3,1fr)}.bar-row{grid-template-columns:92px 1fr 45px}
</style>
</head>
<body>
<main class="shell">
...
<section class="view" id="overview" role="tabpanel" aria-labelledby="overview-tab">
  ...
  <div class="section"><h2>Causal scope</h2><p class="section-lead">The gate separates matched Arm execution evidence from other deployment changes.</p><div class="scope"><div class="node"><b>Same artifact and workload</b><br><span>Observed source hashes must match the contract</span></div><div class="arrow">&rarr;</div><div class="node"><b>One acceleration control changes</b><br><span>All remaining declared controls must stay matched</span></div><div class="arrow">&rarr;</div><div class="node"><b>Executed Arm path</b><br><span>Sampled profile present only in the optimized treatment</span></div></div></div>
</section>
...
if(verification){const profile=verification.performix;const memory=verification.runtime_memory;const reproduced=Boolean(verification.reproduction_checksums);const supporting=verification.supporting_evidence;const publication=verification.preregistration_publication;const checked=verification.checksums.checked+(reproduced?verification.reproduction_checksums.checked:0)+(profile?profile.internal_checksums.checked:0)+(supporting?supporting.checksummed_files:0);const scope=profile&&memory?'capacity, raw quality, runtime experiments, deployment measurements and native Arm Performix bundles':profile?'capacity, raw quality and native Arm Performix bundles':reproduced?'primary and fresh-instance confirmation bundles':'the evidence bundle';const publicationText=publication?` Git object ${publication.git_commit.slice(0,7)} contains the exact plan; its commit time precedes the launch time recorded in experiment metadata. This is a chronology check, not independent AWS attestation.`:'';document.getElementById('verification-detail').textContent=`${checked} files verified across ${scope}. The comparison and decision were derived from bound evidence.${publicationText}`;if(profile){document.getElementById('performix-section').hidden=false;document.getElementById('performix-detail').textContent=`Arm Performix ${profile.enabled.engine_version} measured matched Code Hotspots runs on ${profile.enabled.cpu_names.join(', ')}.`;document.getElementById('performix-disabled').textContent=`${(profile.disabled.kai_sample_share*100).toFixed(0)}% measured kai_* function samples`;document.getElementById('performix-enabled').textContent=`${(profile.enabled.kai_sample_share*100).toFixed(2)}% measured kai_* function samples`;const perfShare=profile.linux_perf_separate_kai_cycle_share;document.getElementById('performix-crosscheck').textContent=Number.isFinite(perfShare)?`Linux perf separately measured ${(perfShare*100).toFixed(2)}% cycle attribution; the two profilers use different units.`:'The release gate evaluates Performix function samples in their native units.';}if(memory){document.getElementById('memory-section').hidden=false;document.getElementById('memory-capacity').textContent=memory.candidate_rps.toFixed(2);document.getElementById('memory-detail').textContent=`The short screen favored a simpler memory recipe, but it failed ${memory.simplification_failures}/${memory.simplification_windows} sustained windows. ArmProof releases the complete thread, allocator, and huge-page recipe that passed ${memory.confirmation_passes}/${memory.confirmation_windows}.`;document.getElementById('memory-control').textContent=`${(memory.baseline_median_p95_ms/1000).toFixed(2)} s median p95 at ${memory.candidate_rps.toFixed(2)} requests/s`;document.getElementById('memory-treatment').textContent=`${(memory.optimized_median_p95_ms/1000).toFixed(2)} s median p95; ${memory.capacity_gain_percent.toFixed(2)}% higher verified traffic floor`;}}
